...
Client --> [ Chat Server (WebSocket) ] --> [ Redis INCR (sequence) ]
                     |                              |
                     v                              v
              [ Redis Pub/Sub ]              [ Cassandra (messages) ]
         (publish to user:{id})
                     |
                     v
              ...
              (if offline)
                     v
              [ Inbox Queue ] --> [ Push Notification Service (APNs/FCM) ]
```

- **Chat Servers**: Stateful WebSocket servers, ~50K connections each. Handle message routing and delivery.
- **Redis Pub/Sub for routing**: With many WS servers, how does WS1 (sender's server) find WS2 (recipient's server)? Each Chat Server subscribes to `user:{user_id}` channels for its connected users. To deliver, publish to `user:{recipient_id}` -- the right server picks it up automatically.
- **Redis**: Per-conversation sequence counters, presence, typing indicators, unread counts.
...
**Canonical 1:1 conversation ID**: For DMs, enforce `participant_1 < participant_2` with a unique index to prevent duplicate conversations between the same pair.
...
## 6. Offline Delivery: Inbox + In-Flight Pattern
When a recipient is offline, messages go to an **inbox** (per-user queue). On reconnect, messages move to **in-flight** and are delivered via WebSocket. The client **ACKs** each message; on ACK, the message is deleted from in-flight. A **reaper** process returns stale in-flight items (no ACK after 60s) back to inbox for redelivery. This is the SQS visibility-timeout pattern -- zero message loss.

## 6.5. Read Receipts: High-Water Mark
Don't track read status per-message. Store `last_read_sequence_number` per user per conversation. All messages with `seq <= last_read_seq` are read. Client **debounces** read events (2-second delay) to avoid spamming writes as the user scrolls.

## 6.6. Multi-Device
Each device has its own inbox and its own delivery status. Messages fan-out to all active device inboxes on send. Each device independently ACKs and tracks its own `last_received_seq`.
...
| WebSocket vs SSE vs polling | WebSocket: bidirectional -- both parties push data (messages, typing, presence). SSE is server-only push (client would need separate send channel). Polling adds latency and overhead. |
